...
class robot:
    ...
    def measurement_prob(self, measurements):
        # importance weights (for calculating the correct measurement)
    ...
    def move(self, motion, tolerance=0.001):  # Do not change the name of this function
        ...
        if abs(steering) > max_steering_angle:
            raise ValueError ('Exceeding maximum steering angle')
        if distance < 0.0:
            raise ValueError ('Cannot moving backwards')
        # make new copy
        res = robot()
        ...
        if abs(turn) < tolerance:
            # consider straight line motion
            res.x = self.x + (distance2 * cos(self.orientation))
            res.y = self.y + (distance2 * sin(self.orientation))
            res.orientation = (self.orientation + turn) % (2.0 * pi)
        else:
            # consider bike model motion
        ...
        return res  # make sure move function returns an instance of the robot class with the correct coordinates.
...
def get_position(p):
    ...
    for i in range(len(p)):
        x += p[i].x
        y += p[i].y
        # orientation is cyclic. By normalizing around the first particle 0=2pi problem
        orientation+=(((p[i].orientation-p[0].orientation+pi)%(2.0*pi))+p[0].orientation-pi)
    # average
    return [x / len(p), y / len(p), orientation / len(p)]

# Code generates the measurements vector
...
# Code prints the measurements associated with generate_ground_truth
...
# Code checks to see if the particle filter localizes the robot to within the desired tolerances of the true position.
